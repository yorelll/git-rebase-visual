...
import { ComposePanelDelivery } from "./composePanelState";
...
export class ComposePanel implements vscode.Disposable {
  ...
  open(payload: ComposePayload): void {
    const readyPayload = this.delivery.update(payload);
    if (!this.panel) {
      this.delivery.reset();
      ...
      this.disposables.push(
        this.panel.webview.onDidReceiveMessage((message) => {
          if (message?.type === "composeReady") {
            const latest = this.delivery.markReady();
            if (latest) this.post({ type: "openCompose", ...latest });
            return;
          }
    ...
    if (readyPayload) this.post({ type: "openCompose", ...readyPayload });
  }
