...
export class ComposePanel implements vscode.Disposable {
  ...
  open(payload: ComposePayload): void {
    ...
    this.post({ type: "openCompose", ...payload });
  }

  post(message: any): void { this.panel?.webview.postMessage(message); }
  visible(): boolean { return !!this.panel && !this.disposed; }
  dispose(): void { this.disposed = true; this.panel?.dispose(); }
