...
export class ComposePanel implements vscode.Disposable {
  ...
  private title(payload: ComposeSessionPayload): string {
    ...
    if (payload.messageOnly) return payload.ai ? "AI 草稿 message · 仅生成，不提交" : "草稿 message · 仅生成，不提交";
...
<h1 id="title">Compose commit message <span class="dirty" aria-label="未保存草稿">●</span></h1><div id="context" class="context"></div><div id="draft-only" class="warning" role="status">仅生成 message，不会提交或修改 rebase；复制后由你自行使用。</div><button id="diff" class="diff-action" type="button" style="display:none">打开目标变更…</button><div id="warning" class="warning" role="alert"></div><div id="error" class="error" role="alert"></div><details><summary id="original-summary">原始 message（展开查看）</summary><pre id="original"></pre></details><details id="trailers"><summary>Trailer（原样保留）</summary><pre id="trailer"></pre></details>
